...
function boot() {
  if (typeof window.XLSX === "undefined" || typeof window.JSZip === "undefined") {
    setStatus("Не вдалося завантажити бібліотеки");
    log("Помилка: зовнішні бібліотеки XLSX або JSZip не завантажилися.");
    elements.processButton.disabled = true;
    return;
  }

  updateMappingCount();
  log("Вебверсія готова. Виберіть XLSX і натисніть «Зібрати ZIP з DOCX».");
...
function handleFileSelection(event) {
  ...
  elements.fileName.textContent = file ? file.name : "Файл ще не вибрано";
}

async function handleProcess() {
  const file = elements.fileInput.files && elements.fileInput.files[0];
  if (!file) {
    setStatus("Потрібно вибрати XLSX");
    alert("Спочатку виберіть XLSX-файл.");
    return;
  }

  setBusy(true);
  setStatus("Триває обробка...");
  log("");
  log("===== Новий запуск =====");
  log(`Вхідний файл: ${file.name}`);
  log(
    elements.addBlankColumn.checked
      ? "Опцію ввімкнено: додати третій порожній стовпець."
      : "Опцію вимкнено: не додавати третій порожній стовпець."
  );

  try {
    const report = await processWorkbook(file, elements.addBlankColumn.checked);
    log("Підсумок обробки");
    log(`Прочитано рядків: ${report.totalRowsRead}`);
    log(`Успішно оброблено: ${report.processedRows}`);
    log(`Створено документів: ${report.documentsCreated}`);
    log(`Архів завантажено: ${report.archiveName}`);
    ...
    if (report.skippedRows.length) {
      ...
      for (const skipped of report.skippedRows) {
        let details = ` - Рядок ${skipped.worksheetRow}: ${skipped.reason}`;
        if (skipped.vesselText) {
          details += ` [Судно: ${skipped.vesselText}]`;
        }
        if (skipped.agencyText) {
          details += ` [Компанія: ${skipped.agencyText}]`;
        }
        log(details);
      }
    }
    ...
    if (report.merges.length) {
      log("Об’єднані компанії:");
      for (const merge of report.merges) {
        log(` - ${merge.fromName} -> ${merge.toName}${merge.persisted ? " (збережено)" : ""}`);
      }
    }

    setStatus("Готово. ZIP завантажено.");
  } catch (error) {
    const message = error instanceof Error ? error.message : "Невідома помилка.";
    log(`Помилка: ${message}`);
    setStatus("Помилка обробки");
    alert("Під час обробки сталася помилка. Подробиці є в журналі.");
  } finally {
    setBusy(false);
  }
}
...
async function processWorkbook(file, addBlankColumn) {
  ...
  if (!firstSheetName) {
    throw new Error("У XLSX не знайдено жодного аркуша.");
  }

  const worksheet = workbook.Sheets[firstSheetName];
  const worksheetData = extractWorksheetData(worksheet);

  log("Пошук заголовків...");
  ...
  for (let rowIndex = headers.headerRowIndex + 1; rowIndex < worksheetData.rows.length; rowIndex += 1) {
    ...
    if (!vesselText || !agencyText) {
      report.skippedRows.push({
        ...
        reason: !vesselText ? "Пропущено: немає назви судна." : "Пропущено: немає агентуючої фірми.",
        dateText,
        vesselText,
        agencyText,
      });
  ...
  for (const group of sortedGroups) {
    const baseName = sanitizeFileName(group.displayName, "Компанія");
...
function detectHeaders(data) {
  ...
  if (best.dateColumn < 0 || best.vesselColumn < 0 || best.agencyColumn < 0) {
    throw new Error("Не вдалося автоматично знайти колонки «Дата», «Назва» та «Агентуюча фірма».");
  }
...
function isDateHeader(value) {
  return containsAny(value, ["дата", "date"]);
}
...
function isVesselHeader(value) {
  return value === "назва" ||
    value === "название" ||
    value === "судно" ||
    containsAny(value, ["vessel", "ship"]) ||
    (containsAny(value, ["назв", "наз"]) && containsAny(value, ["суд", "vessel", "ship"]));
}

function isAgencyHeader(value) {
  const hasAgencyWord = containsAny(value, ["агент", "agency", "agent"]);
  const hasFirmWord = containsAny(value, ["фірм", "фирм", "компан", "company", "firm"]);
  return hasAgencyWord && hasFirmWord;
}
...
function buildDocumentXml(group, addBlankColumn) {
  ...
  xml += "<w:tr>";
  xml += buildCell("Назва", addBlankColumn ? 6200 : 7200, true, true);
...
function buildReportText(report) {
  const lines = [
    "Autovessel Web - звіт обробки",
    "",
    `Прочитано рядків: ${report.totalRowsRead}`,
    `Успішно оброблено: ${report.processedRows}`,
    `Створено документів: ${report.documentsCreated}`,
    "",
    "Файли:",
...
function askSimilarityDecision(match) {
  elements.decisionBody.textContent =
    `Знайдено схожі назви компаній:\n\n"${match.candidate}" і "${match.existing}".\n\nЩо зробити?`;
...
function handleClearMappings() {
  if (!confirm("Очистити всі збережені об’єднання компаній у браузері?")) {
    return;
  }

  const storage = new RegistryStorage();
  storage.clear();
  log("Збережені об’єднання очищено.");
  setStatus("Пам’ять об’єднань очищено");
}
...
function containsAny(haystack, needles) {
  return needles.some((needle) => containsInsensitive(haystack, needle));
}
